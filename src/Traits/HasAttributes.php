<?php
/**
 * Nom du fichier : HasAttributes.php
 * Projet : AlfrescoAPI
 * Date : 29/08/2017
 */
...
trait HasAttributes
{
    ...
    public function getAttribute($key)
    {
        ...
        if ($this->hasMacros($key)) {
            return $this->getAttributeWithMacro($key);
        }
    ...
    public function hasMacros($key = null)
    {
        if (!$key) {
            return config('alfresco.macros.'.static::class, false);
        }
        return config('alfresco.macros.'.static::class.'.'.$key, false);
    }

    public function getAttributeWithMacro($key)
    {
        $macros = config('alfresco.macros.' . static::class . '.' . $key);
        if (!is_array($macros)) {
            return Arr::get($this->getAttributes(), config('alfresco.macros.'.static::class.'.'.$key), null);
        }
        foreach ($macros as $item) {
            if (Arr::has($this->getAttributes(), $item)) {
                return Arr::get($this->getAttributes(), $item, null);
            }
        }
    }
}